...
const PlayerTpBar = ({ tp }) => {
  ...
  $(labelSvg.node).addClass("label").appendTo("#tp-bar");
  ...
  $(barSvg.node).appendTo("#tp-bar");

  const { svg: hpLabelSvg, setText: setTp } = Label({
    text: value,
    color: "blue",
    fontProps: {
      "font-size": "16px",
  ...
  $(hpLabelSvg.node).addClass("number").appendTo("#tp-bar");

  const set = (tp) => {
    setValue(tp, maxValue);
    setTp(tp);
  };
