...
const PlayerMpBar = ({ mp, maxMp }) => {
  let value = mp ? mp : 1000,
    maxValue = maxMp ? maxMp : 1000;
  ...
  $(barStub).appendTo("#mp-bar");
  $(barStub).removeClass("left").addClass("right").appendTo("#mp-bar");
  ...
  $(labelSvg.node).addClass("label").appendTo("#mp-bar");
  ...
  $(barSvg.node).appendTo("#mp-bar");

  const { svg: mpLabelSvg, setText: setMp } = Label({
    text: value,
    color: "green",
    fontProps: {
      "font-size": "16px",
  ...
  $(mpLabelSvg.node).addClass("number").appendTo("#mp-bar");
